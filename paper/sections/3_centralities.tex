...
In localised analysis, reachable node counts vary by origin. Because \emph{Closeness} and \emph{Normalised Closeness} divide by total distance, variation in node count conflates two effects: whether a given denominator is larger due to more nodes being reachable, or whether reachable nodes are more distant on average. This undermines comparability across locations. In the broader network analysis field, it has been shown that \emph{Harmonic Closeness} centrality \cite{Marchiori2000, Rochat2009} scales more suitably for sub-graphs, with the difference being that the division happens prior to the summation
