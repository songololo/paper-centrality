...
Software developers have addressed this issue in practice, though implementations vary. \texttt{DepthmapX} \cite{turner_depthmapx_2020}, \texttt{Place Syntax Tool} \cite{stahle_place_2023}, \texttt{cityseer} \cite{simons_cityseer_2023}, \texttt{NetworkX} \cite{hagberg_networkx_2008}, and \texttt{igraph} \cite{csardi_igraph_2006} all implement \emph{Improved Closeness}, \emph{Harmonic Closeness}, or both. Other specialised packages take different approaches: \texttt{sDNA} \cite{cooper_sdna_2020} outputs a gravity-like measure (NQPD), while the \texttt{Urban Network Analysis} toolbox \cite{Sevtsuk2012} provides a \emph{Gravity Index}. Researchers using these tools obtain results from the implemented formulation regardless of which formula is cited in accompanying publications.

The reproducibility problem affects researchers who implement closeness centrality from cited formulas, attempt to replicate published studies without access to original code, or review or teach network centrality methods based on commonly cited formulations. In these cases, implementing the cited \emph{Normalised Closeness} formula for localised analysis will yield different results from the original study, potentially with reversed sign or negligible magnitude.
