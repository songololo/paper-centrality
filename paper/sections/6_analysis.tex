...
\subsection{Comparisons to Landuse Accessibility}
...
The spatial autocorrelation analysis (Table~\ref{tab:morans_i}) reveals a divergence between closeness formulations. Measures that behave as intended --- \emph{Improved Closeness}, \emph{Gravity}, and \emph{Harmonic Closeness} --- exhibit high Moran's $I$ values ($I \approx 0.79$--$0.84$), consistent with the expected smooth spatial clustering associated with closeness centrality. Nearby street segments sharing similar network context have similarly pronounced centrality values under these formulations. In contrast, \emph{Closeness} and \emph{Normalised Closeness} exhibit lower spatial autocorrelation ($I \approx 0.18$--$0.2$ at 500m). Reflecting these differences in spatial coherence, the effective sample size varies from approximately 3,700 for well-normalized measures with high spatial coherence ($I \approx 0.84$) to 27,000 for \emph{Closeness} ($I \approx 0.21$), compared to the nominal $N = 42,167$ segments.

The block bootstrap confidence intervals (Table~\ref{tab:bootstrap_ci}) confirm that \emph{Closeness} exhibits strong \emph{negative} correlations with landuse intensity ($\rho \approx -0.7$ for 500m, 95\% CI: $[-0.76, -0.64]$), a pathological result indicating that the measure assigns higher values to less accessible locations. \emph{Normalised Closeness} ($N^1$) shows correlations near zero ($\rho \approx -0.11$ for 500m, 95\% CI: $[-0.1576, -0.0659]$), confirming that simple normalisation fails to correct the underlying issue. In contrast, \emph{Improved Closeness} ($\rho \approx +0.67$ at 500m, 95\% CI: $[+0.60, +0.72]$), \emph{Gravity}, and \emph{Harmonic Closeness} achieve consistently strong positive correlations.

\subsection{Summary}

The empirical results confirm the theoretical hypothesis presented in Section~\ref{closeness_centrality}. \emph{Normalised Closeness}, despite being widely cited in the street network analysis literature, shows weak or negative associations with landuse accessibility and origin-destination trip volumes when computed for localised analyses. In contrast, \emph{Improved Closeness} and \emph{Harmonic Closeness} --- which scale appropriately across varying numbers of reachable nodes --- demonstrate consistently stronger associations. This supports our contention that the divergence between cited formulations and those implemented in computational packages reflects a meaningful methodological distinction rather than notational variation.

\subsection{Limitations}

This study is based on a single city case study, and the specific correlation magnitudes may differ for other urban contexts. Madrid's dense, walkable historic core with high landuse intensities distributed across the network may favour closeness-like measures over betweenness; contexts with more pronounced linear high streets (e.g., London) or studies based on detailed pedestrian counts might show different relative performance between measures and stronger associations for \emph{geometric} distance variants. The analysis is cross-sectional and does not capture temporal dynamics in landuse or travel patterns.
