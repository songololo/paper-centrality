...
Network centralities are typically associated with land-use intensities \cite{Porta2009} and travel volumes \cite{Hillier2007}: locations with greater network accessibility tend to attract more activity and land-use intensity. We therefore expect positive correlations for well-behaved closeness formulations; negative or negligible correlations would indicate counter-intuitive scaling behaviour. Madrid was selected for this case study due to the availability of a high-quality street network dataset accompanied by high-resolution land-use premises information and origin-destination travel data. For full reproducibility, access to the datasets, code workflow, and related data preparation notes is provided in an open code repository.
...
\begin{enumerate}
  ...
  \item \emph{Closeness $N^{1.2}$} raises the node count in the numerator to the power of 1.2. This form, referred to as ``normalised'' least angular integration (NAIN) in the space syntax community, was derived empirically in \cite{Hillier2012} with the aim of harmonising values across different distance radii.
