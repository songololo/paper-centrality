...
\end{equation}
is the sum of shortest paths between all $(j, k)$ node pairs passing through node $i$ \cite{Freeman1977}. For street networks, it conveys how likely a street is to be traversed by people travelling between other locations. The Space Syntax measure \emph{Choice} is similar in intent but differs in how it is computed \cite{stahle_place_2023}. Localised \emph{Betweenness} considers only $(j, k)$ pairs within threshold cutoff distances.
