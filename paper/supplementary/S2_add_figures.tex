...
\begin{figure*}[htb]
  ...
  \caption{PCA loadings matrix. Input variables were preprocessed using \texttt{PowerTransformer} (reshaping to normal distribution) and \texttt{StandardScaler} (mean centering and variance scaling) from \texttt{scikit-learn} \cite{Pedregosa2011} prior to PCA.}\label{fig:pca_loadings}
\end{figure*}

\subsubsection{Angular and Length-Weighted Centrality Results}

The following figures present results for angular (geometric distance) and length-weighted centrality variants. These complement the metric distance results in the main text and demonstrate that the observed patterns hold across different distance formulations.
